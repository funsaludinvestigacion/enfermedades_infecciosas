...
# Filter based on disease if needed


# Collapse symptoms based on count
symptom_summary_wide <- symptom_summary%>%
...
tile_plot <- symptoms_summary_counts %>%
  ggplot(aes(x = epiweek1, y = Síntoma)) +
  geom_tile(aes(fill = Count), color='black') +
  facet_wrap(~ visit_type, ncol = 1, scales = "free_y") +  # Facet by Tipo de Muestra
  labs(
    title="Número de individuos experimentando un dado síntoma \n por semana despues de una prueba de PCR",
    x = "Semana",
    y = "",
    fill= ""
  ) +
